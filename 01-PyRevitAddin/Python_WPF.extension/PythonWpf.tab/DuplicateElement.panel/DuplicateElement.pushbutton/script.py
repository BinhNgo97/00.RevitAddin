...
class DuplicateElementWindow(forms.WPFWindow):
    ...
    def wire_events(self, view_model):
        ...
        try:
            ...
            if hasattr(self, 'cmbFaElType'):
                ...
                # enable type-ahead filtering via KeyUp on editable ComboBox
                try:
                    self.cmbFaElType.IsEditable = True
                    self.cmbFaElType.IsTextSearchEnabled = False
                except:
                    pass
                try:
                    self.cmbFaElType.KeyUp += self.on_fael_key_up
                except:
                    # fallback: SelectionChanged will refresh to remove filter if needed
                    pass
            ...
            # Buttons below already have Command bindings in XAML; avoid double-execution by not wiring Click here:
            # - btnSelectExcel (SelectExcelCommand)
            # - btnImportData (ImportDataCommand)
            # - btnRefresh (RefreshDataCommand)
            # - btnUpdateSelected (UpdateAllCommand)
            # - btnCreateAll (CreateAllNewCommand)
            # Only initialize display text for btnSelectExcel
    ...
    def on_fael_key_up(self, sender, e):
        """Filter FaElTypes by typed text; opens dropdown to show matches."""
        try:
            # Get current text from combo
            text = ""
            try:
                text = str(self.cmbFaElType.Text or "").strip().lower()
            except:
                text = ""
            ...
            cv = None
            try:
                cv = CollectionViewSource.GetDefaultView(self._view_model.FaElTypes)
            except:
                cv = None
            if cv is None:
                return
            def _predicate(item):
                try:
                    ...
                    return True
            try:
                cv.Filter = _predicate
                cv.Refresh()
                # keep dropdown open and show filtered items
                try:
                    if not self.cmbFaElType.IsDropDownOpen:
                        self.cmbFaElType.IsDropDownOpen = True
                except:
                    pass
            except:
                pass
