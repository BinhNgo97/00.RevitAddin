...
# Thêm thư mục PythonWpf.tab vào sys.path để import aGeneral
_tab_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
if _tab_dir not in sys.path:
    sys.path.append(_tab_dir)
...
class ConditionRow(ViewModel_BaseEventHandler):
    ...
    def Category(self, v):
        if v != self._category:
            self._category = v
            # Rebuild snapshot rules with default template for new category
            if self._vm:
                new_rules = self._vm._build_default_rules(v)
                self._rules = new_rules
                # Update Bang 2 only if THIS condition is currently selected
                if self._vm._selected_condition is self:
                    self._vm._current_rules.Clear()
                    for r in new_rules:
                        self._vm._current_rules.Add(RuleRow.from_dict(r.to_dict()))
            else:
                self._rules = []
        else:
            self._category = v
...
class ModelByCadViewModel(ViewModel_BaseEventHandler):
    ...
    def __init__(self):
        ...
        # Bảng 3 – mặc định 1 dòng rỗng
        self._conditions.Add(ConditionRow(self))
    ...
    def copy_condition(self, source_row):
        """Tạo bản sao của ConditionRow, deep-copy Rules, thêm vào Bảng 3."""
        new_row = ConditionRow(self)
        new_row._condition_name = source_row._condition_name + '_copy'
        new_row._file_name      = source_row._file_name
        new_row._category       = source_row._category  # set trực tiếp, không reset rules
        new_row._rules          = [RuleRow.from_dict(r.to_dict()) for r in source_row._rules]
        self._conditions.Add(new_row)
        return new_row
    ...
    def _build_default_rules(self, category):
        """Tra ve list[RuleRow] mac dinh theo category (khong sua CurrentRules truc tiep)."""
        cat = category.lower() if category else ''
        if 'column' in cat or 'foundation' in cat or 'footing' in cat:
            return [RuleRow('Layer Name', 'Equal', '')]
        elif 'framing' in cat or 'beam' in cat:
            return [
                RuleRow('Min Beam Distance', 'Equal', ''),
                RuleRow('Max Beam Distance', 'Equal', ''),
                RuleRow('Length',            'is greater than', ''),
                RuleRow('Text Layer',        'Equal', ''),
                RuleRow('Layer Name',        'Equal', ''),
            ]
        elif 'wall' in cat:
            return [
                RuleRow('Length',     'is greater than', ''),
