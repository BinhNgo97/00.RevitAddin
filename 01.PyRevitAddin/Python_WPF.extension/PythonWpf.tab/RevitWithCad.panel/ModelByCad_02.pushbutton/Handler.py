...
def on_load_file(sender, e):
    """
    Load 1 hoặc nhiều file DWG vào ViewModel.
    Với mỗi file: kết nối AutoCAD → mở file → extract layers + elements → cache.
    """
    window = sender.Tag
    ...
    dlg.Multiselect = True   # Cho phép chọn nhiều file
    ...
    if loaded:
        vm.Status = u"Da load: " + u", ".join(loaded)
        # Cập nhật dropdown file trong Bảng 1 binding
        _refresh_cond_file_dropdown(window)
    else:
        MessageBox.Show(u"Khong the load bat ky file nao.", u"Load File CAD")


def _refresh_cond_file_dropdown(window):
    """Force cập nhật ComboBox file trong Bảng 1."""
    vm = window.DataContext
    cb = window.CbCondFile
    cur = vm.CondFile
    cb.ItemsSource = vm.LoadedFileNames
    if cur in list(vm.LoadedFileNames):
        cb.SelectedItem = cur
...
def _populate_default_rules(vm, category):
    """Thêm rules gợi ý vào Bảng 2 theo category."""
...
def on_remove_condition(sender, e):
    """Xóa condition đang chọn ở Bảng 3."""
    window = sender.Tag
    vm     = window.DataContext
    if vm.SelectedCondition is None:
        MessageBox.Show(u"Chua chon condition nao trong Bảng 3.", u"Remove")
        return
